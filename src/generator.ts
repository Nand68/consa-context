...
function validateConfig(config: ConsaConfig): void {
  ...
  if (!/^[A-Z][a-zA-Z0-9]*Context$/.test(config.contextName)) {
    console.warn(
      `⚠️  Warning: contextName should follow PascalCase and end with 'Context' (e.g., 'ThemeContext')`
    );
  }
  ...
  config.states?.forEach((state) => {
    if (!/^[a-z][a-zA-Z0-9]*$/.test(state.name)) {
      throw new Error(
        `❌ State name '${state.name}' should be camelCase (e.g., 'theme', 'isOpen')`
      );
    }
  });
  ...
  config.variables?.forEach((variable) => {
    if (!/^[a-z][a-zA-Z0-9]*$/.test(variable.name)) {
      throw new Error(
        `❌ Variable name '${variable.name}' should be camelCase`
      );
    }
  });
  ...
  config.functions?.forEach((func) => {
    if (!/^[a-z][a-zA-Z0-9]*$/.test(func.name)) {
      throw new Error(
        `❌ Function name '${func.name}' should be camelCase`
      );
    }
  });
}
...
export default {
  ...
  try {
    ...
    const config: ConsaConfig = configModule.default || {};
    ...
    const stateDeclarations = states
      .map(
        (s) =>
          `const [${s.name}, set${capitalize(s.name)}] = useState<${
            s.type
          }>(${formatInitialValue(s.initialValue)});`
      )
      .join("\n  ");
    ...
    const interfaceProps = [
      ...states.map(
        (s) =>
          `${s.name}: ${s.type};\n  set${capitalize(
            s.name
          )}: React.Dispatch<React.SetStateAction<${s.type}>>;`
      ),
